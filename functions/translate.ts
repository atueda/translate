...
export const def = DefineFunction({
  ...
  input_parameters: {
    ...
    //required: ["googleAccessTokenId", "channelId", "messageTs"],
...
export default SlackFunction(def, async ({ inputs, client, env }) => {
  ...
  // Google Translate
  if (env.TRANSLATE_API === "GOOGLE") {
    console.log("Google Translate");

    //use translate v2. (Basic Edition)
    const url =
      `https://translation.googleapis.com/language/translate/v2?key=${env.GOOGLE_API_KEY}`;

    const body = new URLSearchParams();
    body.append("q", translationTarget.text);
    body.append("target", inputs.lang.toUpperCase());
    ...
    if (apiResponseStatus != 200) {
      ...
      const error =
        `Google API call failed - status: ${apiResponseStatus}, body: ${body}`;
      return { error };
    }
    ...
    if (
      !translationResult ||
      translationResult["data"]["translations"][0]["translatedText"] === 0
    ) {
